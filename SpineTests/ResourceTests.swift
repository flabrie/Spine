...
class ResourceTests: XCTestCase {
	...
	func testEncoding() {
		...
		do {
			let encodedData = try NSKeyedArchiver.archivedData(withRootObject: foo, requiringSecureCoding: false)
			let decodedFoo: AnyObject? = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(encodedData) as AnyObject
